...
// Route to add multiple designer products in bulk
router.post("/bulk", async (req, res) => {
    try {
        ...
        const savedProducts = await Designer.insertMany(products);
...
// Route to get all designer products
router.get("/", async (req, res) => {
    try {
        const products = await Designer.find();
...
// Route to get a single designer product by ID
router.get("/:id", async (req, res) => {
    try {
        const product = await Designer.findById(req.params.id); // Using MongoDB's auto-generated _id
        if (!product) {
